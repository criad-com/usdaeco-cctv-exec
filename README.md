...
This library has no example directory or result to publish. The native consumer evaluates the CCTV [lobby](https://github.com/criad-com/usdaeco-cctv/blob/v0.5.2/examples/lobby.usda) before and after a pan edit. It checks eight sensor records against both the Python computation and baked derived layers. [Previous v0.2.1 acceptance](docs/republish-acceptance.json) records that release's native run. [Previous release evidence](docs/release-acceptance.json) records 21/21 native checks and 8/8 baked records at v0.1.3; this is historical evidence, not a current native pass.
...
path:../usdaeco-toolchain`, `--override-input usdaeco-core path:../usdaeco-core`
and `--override-input usdaeco-cctv path:../usdaeco-cctv` to `nix develop` or
...
Requires `usdAeco >=0.9.2,<1.0` and `usdAecoCctv >=0.5.2,<0.6`. [Exact pins](dependencies.json) select core v0.9.2, CCTV v0.5.2 and toolchain v0.3.8 for train aeco-0.7.0. [Family board](https://github.com/criad-com/usdaeco-board).
...
Version **0.2.3**, train **aeco-0.7.0**, public names under `github.com/criad-com`: **54 checks, 0 failed, 21 not run; 10 pytest cases passed, 14 native cases deselected with NOT RUN reasons**. Structure checks use toolchain v0.3.8. The usd-dev consumer and installed exec plugin descriptor are missing. The single offline Nix attempt could not access the daemon socket under network isolation; Nix remains **not proven**. [Previous v0.2.1 migration evidence, applicable S-rules and deviations](docs/migration.md). The native computations are unchanged from v0.1.3. A stock viewer does not automatically request these computations; a viewer integration remains outside this release.
